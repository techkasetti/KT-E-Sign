...
export default class AnalyticsDashboard extends LightningElement {
    ...
    wiredAnalytics(result) {
        ...
        if (result.data) {
            ...
            this.isLoading = false;
            this.showToast('Error', 'Failed to load analytics: ' + result.error.body?.message, 'error');
        }
    }
    ...
    get actionBreakdownList() {
        ...
        return Object.keys(this.dashboardData.userActivityMetrics.actionBreakdown).map(key => ({
            name: key,
    ...
    get operationMetricsList() {
        ...
        return Object.keys(this.dashboardData.performanceMetrics.operationMetrics).map(key => {
        ...
        this.chartInstances.statusChart = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [
                    {
                        data: data,
                        backgroundColor: ['#1B96FF', '#06A59A', '#FFB75D', '#FE5F55', '#C23934'],
                        borderWidth: 2,
                        borderColor: '#FFFFFF'
                    }
                ]
    ...
    formatActionLabel(actionKey) {
        return actionKey.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
    }

    formatOperationLabel(operationKey) {
        return operationKey.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
    }
